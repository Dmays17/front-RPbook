...
const [updateForm,setUpdatedForm]=useState({})
  useEffect(()=>{
    axios
    .get("https://rpbookapi.herokuapp.com/api/Mybook")
    .then((res) => {
        console.log(res)
        setForm(res.data)
...
const deletePost=(id)=>{
    ...
    axios.delete(`https://rpbookapi.herokuapp.com/api/delete/${id}`)
...
const saveUpdateForm=()=>{
    axios
        .put(`https://rpbookapi.herokuapp.com/api/update/${updateForm._id}`,updateForm)
        .then(res => console.log(res))
        .catch((err)=> console.log(err))
